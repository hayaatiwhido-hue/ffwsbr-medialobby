...
(() => {
  const API = {
    ...
    membros: window.__FFWS.u('u5'),
    fotos: window.__FFWS.u('u6')
  };
  ...
  async function carregarTudo() {
    const [dominiosRaw, estilosRaw, equipesRaw, assetsRaw, membrosRaw, fotosRaw] = await Promise.all([
  ...
  window.MEDIA_LOBBY_API = API;
  window.MEDIA_LOBBY_STATUS = { statusTipo, maintenance, falsy, truthy };
  window.__FFWS_DATA_READY = carregarTudo().catch(error => {
